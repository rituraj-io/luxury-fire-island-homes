...
// Current Buy — landing section for /buy. Cream band with a script heading,
// filter row (search / sort / map), and a 3-up grid of for-sale property
// cards. LOAD MORE reveals more results. Search/sort/map controls are visual
// placeholders — wiring will follow once the search backend is connected.
...
function toCard(p: FeaturedProperty): BuyCard {
	return {
		id: p.id,
		code: p.title,
		price: p.priceLabel,
...
export default function CurrentBuy({ items }: Props) {
				...
				<Reveal y={DISTANCE.text} duration={DURATION.text} className="flex flex-col items-center text-center">
					...
					<div className="mt-8 flex w-full max-w-[720px] flex-col gap-3 sm:flex-row sm:items-stretch">
						<label className="min-w-0 flex-1">
							<span className="sr-only">Search homes for sale</span>
